//! image_ops — port verbatim `kuron_native/rust/src/image_ops.rs` (task 5.1).
//! chunk_webtoon / build_mosaic / compress_page: bytes in → bytes out, zero state.
//! Error `String` mobile dipetakan ke `AppError::Internal` (konvensi desktop).
//! Byte-identik vs mobile (crate `image` 0.25.10) — diuji golden SHA-256.
...
fn decode(data: &[u8]) -> Result<DynamicImage, AppError> {
    let img = image::load_from_memory(data).map_err(|e| err(format!("decode failed: {e}")))?;
...
fn encode_jpeg_rgb(img: &DynamicImage, quality: u8) -> Result<Vec<u8>, AppError> {
    ...
    JpegEncoder::new_with_quality(&mut buf, quality)
        ...
        .map_err(|e| err(format!("encode failed: {e}")))?;
    Ok(buf.into_inner())
}
...
pub fn chunk_webtoon(data: &[u8], max_chunk_h: u32) -> Result<Vec<Vec<u8>>, AppError> {
...
pub fn build_mosaic(data: &[u8], boxes: &[(u32, u32, u32, u32)]) -> Result<Vec<u8>, AppError> {
...
pub fn compress_page(data: &[u8], max_dim: u32) -> Result<Vec<u8>, AppError> {
    let decoded = match decode(data) {
        Ok(img) => img,
...
mod tests {
    ...
    fn dimension_guard_rejects() {
        ...
        bogus.extend_from_slice(&[
            0xFF, 0xC0, 0x00, 0x0B, 0x08, 0x00, 0x64, 0x00, 0x64, 0x03, 0x01, 0x11, 0x00, 0x02,
    ...
    const REAL_STRIP: &[u8] = include_bytes!("../../../test/fixtures/image-webtoon.jpeg");
    ...
    // ── Golden SHA-256 — byte-identik vs `kuron_native/rust` (image 0.25.10) ──
    // Hash dihasilkan dari port mobile pada fixture & input sintetis yang sama.

    #[test]
    fn golden_real_strip_byte_identical() {
        let chunks = chunk_webtoon(REAL_STRIP, 1280).unwrap();
        assert_eq!(chunks.len(), 4);
        let expected = [
            "8f0026d50aa9da69a2891f875efdc7c64394433779c9ea870011039298725b0d",
            "45b52bd6dc840edf96170819f33cb3a689a9f954aed8a3cb620f116f643873a5",
            "1bd2072e0603ad00d2a3d5407a778f80bd1cec650e61efb00295b25864e16ce6",
            "8f4d1eb5e68035b48669d15386107986df1d5eb448559d6e08623153b1c25117",
        ];
        let expected_len = [205376usize, 173390, 194801, 149836];
        for (i, c) in chunks.iter().enumerate() {
            assert_eq!(
                sha256_hex(c),
                expected[i],
                "chunk {i} bytes drift vs mobile"
            );
            assert_eq!(c.len(), expected_len[i], "chunk {i} len drift");
        }
        ...
        let mosaic = build_mosaic(REAL_STRIP, &boxes).unwrap();
        assert_eq!(
            sha256_hex(&mosaic),
            "8b46513b9b7f1babf806135e9dfe6e0eca91a74d02b9a4d2c0fa4799e7b64f34"
        );
        assert_eq!(mosaic.len(), 114300);

        let compressed = compress_page(REAL_STRIP, 1280).unwrap();
        assert_eq!(
            sha256_hex(&compressed),
            "253bbb7efdf4a07befd6526bef0c2cc26d946df7bb10a9dc28e13de99976b392"
        );
        assert_eq!(compressed.len(), 76164);
    }

    #[test]
    fn golden_synthetic_byte_identical() {
        let synth = test_image(400, 2000);
        let chunks = chunk_webtoon(&synth, 1280).unwrap();
        assert_eq!(chunks.len(), 2);
        assert_eq!(
            sha256_hex(&chunks[0]),
            "c496c81c3f7c32936ed5ba4c9767457e4db85901bb32fdde807da461b8892e28"
        );
        assert_eq!(
            sha256_hex(&chunks[1]),
            "71b8d1e03c69568c29c06ca4af28af3f6fbfbc76748e5931b100404e11cb7a16"
        );

        let mosaic = build_mosaic(&synth, &[(10, 10, 50, 40), (80, 100, 60, 30)]).unwrap();
        assert_eq!(
            sha256_hex(&mosaic),
            "1903ea8242596febcebc87680bc4d3235f947c87f77fbce1094225709da72849"
        );
    }
}
